...
Er zijn twee manieren om metriek in de recepten te brengen. Één moet de metriek van de standaardevaluatie gebruiken die door SDK wordt verstrekt en andere moet de metriek van de douaneevaluatie schrijven.
...
Het doel van de methode `split()` is gegevens in te voeren en een training en een testdataset uit te voeren. In ons voorbeeld voert de methode `split()` gegevens in met behulp van de SDK van `DataSetReader` en wist vervolgens de gegevens door niet-verwante kolommen te verwijderen. Hierna worden extra functies gemaakt op basis van bestaande onbewerkte functies in de gegevens.
