...
| ontploffen | Splitst de tekenreeks op basis van een regex en retourneert een array met onderdelen. Kan optioneel regex opnemen om de tekenreeks te splitsen. Standaard wordt de splitsing omgezet in &quot;,&quot;. De volgende scheidingstekens **need** om met `\` te worden ontsnapt: `+, ?, ^, |, ., [, (, {, ), *, $, \` Als u meerdere tekens opneemt als scheidingsteken, wordt het scheidingsteken behandeld als een scheidingsteken voor meerdere tekens. | <ul><li>TEKENREEKS: **Required** De tekenreeks die moet worden gesplitst.</li><li>REGEX: *Optioneel* De reguliere expressie die kan worden gebruikt om de tekenreeks te splitsen.</li></ul> | explode(STRING, REGEX) | explode(&quot;Hallo, daar!&quot;, &quot; &quot;) | `["Hi,", "there"]` |
...
| split | Splitst een invoertekenreeks op een scheidingsteken. Het volgende scheidingsteken **moet** worden beschermd met `\`: `\`. Als u meerdere scheidingstekens opneemt, wordt de tekenreeks gesplitst op **een willekeurige** van de scheidingstekens die in de tekenreeks aanwezig zijn. | <ul><li>INVOER: **Required** De invoertekenreeks die wordt gesplitst.</li><li>SCHEIDINGSTEKEN: **Required** The string that is used to split the input.</li></ul> | split(INPUT, SEPARATOR) | split(&quot;Hello world&quot;, &quot; &quot;) | `["Hello", "world"]` |
...
| contains_key | Controleert of het object bestaat in de brongegevens. | <ul><li>INVOER: **Required** De weg om te controleren als het binnen de brongegevens bestaat.</li></ul> | contains_key(INPUT) | contains_key(&quot;evars.evar.field1&quot;) | true |
