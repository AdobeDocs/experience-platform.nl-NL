...
Voor meer informatie, bezoek [Overzicht van de Werkruimte van de Wetenschap van Gegevens](../../data-science-workspace/home.md).
...
| [!DNL Decisioning]REST-API’s  | [!DNL Decisioning Service] kan worden aangehaald gebruikend eenvoudige REST API om de Volgende Beste Aanbieding voor een bepaald individu te krijgen. Een metriek-API kan worden gebruikt voor het controleren van propositie- en plafondwaarden in real-time. |
| Beslissingsgebeurtenissen streamen naar [!DNL Data Lake] en [!DNL Query Service] | De [!DNL Decisioning Service] maakt automatisch gegevenssets om alle XDM-beslissingsgebeurtenissen automatisch te streamen in [!DNL Data Lake]. De datasets zijn dan beschikbaar voor analyse en rapportering gebruikend [!DNL Query Service]. |
...
* Het aanbiedingsgegevensmodel is niet toegankelijk via de [!DNL Schema Registry] en kan daarom slechts op beperkte wijze worden uitgebreid. Het modelschema heeft ingebouwde structuren om de gehechtheid van douanegegevens toe te staan. In de toekomst kunt u een basis-XDM-modelklasse uitbreiden om uw eigen aangepaste beslissingsdomeinen te definiëren.
...
[!DNL Query Service] Biedt de mogelijkheid om standaard-SQL te gebruiken voor het uitvoeren van query&#39;s op gegevens in Adobe Experience Platform ter ondersteuning van vele verschillende gebruikstoepassingen voor analyse en gegevensbeheer. Het is een serverloos hulpmiddel dat u toestaat om zich bij om het even welke datasets in [!DNL Data Lake] aan te sluiten en de vraagresultaten als nieuwe dataset voor gebruik in het melden, [!DNL Data Science Workspace], of voor opname in [!DNL Profile Service] te vangen.

U kunt [!DNL Query Service] gebruiken om gegevensanalysecosystemen te bouwen, die tot een beeld van consumenten over hun diverse interactiekanalen leiden. Deze kanalen kunnen zijn:
