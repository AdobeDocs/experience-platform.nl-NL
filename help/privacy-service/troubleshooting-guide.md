...
U kunt details over een bepaalde baan terugwinnen door API of [!DNL Privacy Service] gebruikersinterface te gebruiken.

### De API gebruiken

Als u de status van een bepaalde taak wilt ophalen met behulp van de [!DNL Privacy Service] API, vraagt u het hoofdeindpunt (`GET /`) aan met de id van de taak in het aanvraagpad. Zie de sectie over het [controleren van de status van een taak](api/privacy-jobs.md#check-the-status-of-a-job) in de handleiding voor [!DNL Privacy Service] ontwikkelaars voor meer informatie.

### De gebruikersinterface gebruiken

Alle actieve taakverzoeken worden vermeld in de widget **[!UICONTROL Taakverzoeken]** op het [!DNL Privacy Service] UI-dashboard. De status voor elke taakaanvraag wordt weergegeven onder de kolom **[!UICONTROL Status]** . Raadpleeg de gebruikershandleiding bij de [Privacy Service voor meer informatie over het weergeven van taakaanvragen in de gebruikersinterface](ui/user-guide.md).

## Hoe kan ik de resultaten van mijn voltooide privacytaken downloaden?

De [!DNL Privacy Service] API en de gebruikersinterface bieden beide methoden voor het downloaden van de resultaten van voltooide taken in ZIP-indeling.
...
Zie de sectie over het [opzoeken van een taak aan de hand van de id](api/privacy-jobs.md#check-the-status-of-a-job) in de handleiding voor [!DNL Privacy Service] ontwikkelaars voor meer informatie.
...
In de volgende tabel worden enkele veelvoorkomende fouten in [!DNL Privacy Service]de code beschreven, met beschrijvingen die u helpen bij het oplossen van de problemen.
...
| Ongeldige naamruimte | Een opgegeven naamruimte voor de identiteit van een gebruiker-id is ongeldig. Zie de sectie over [standaardnaamruimten](./api/appendix.md#standard-namespaces) voor identiteiten in de [!DNL Privacy Service] ontwikkelaarsgids bijlage voor een lijst met geaccepteerde naamruimten. Als u een aangepaste naamruimte gebruikt, moet u ervoor zorgen dat de `type` eigenschap van de id is ingesteld op &#39;custom&#39;. |
