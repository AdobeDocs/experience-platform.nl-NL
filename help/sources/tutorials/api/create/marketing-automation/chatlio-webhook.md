...
# Een bronverbinding en gegevensstroom maken voor [!DNL Chatlio] de Flow Service API gebruiken
...
Het volgende leerprogramma begeleidt u door de stappen om een bronverbinding en een dataflow tot stand te brengen [[!DNL Chatlio]](https://chatlio.com/) gebeurtenisgegevens naar Adobe Experience Platform met de [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Verbinden [!DNL Chatlio] naar Platform met de [!DNL Flow Service] API {#connect-platform-to-flow-api}

Hieronder worden de stappen beschreven die u moet uitvoeren om een bronverbinding en een gegevensstroom te maken om uw [!DNL Chatlio] gebeurtenisgegevens naar Experience Platform.
...
| `data.format` | Het formaat van de [!DNL Chatlio] gegevens die u wilt invoeren. Momenteel is de enige ondersteunde gegevensindeling `json`. |
...
Met de volgende aanvraag wordt een doelverbinding gemaakt voor [!DNL Chatlio]:
...
| `data.format` | Het formaat van de [!DNL Chatlio] gegevens die u wilt invoeren. |
...
De laatste stap op weg naar het verzamelen van gegevens van [!DNL Chatlio] aan Platform moet een gegevensstroom creëren. Momenteel zijn de volgende vereiste waarden voorbereid:
...
Een succesvolle reactie keert informatie over uw gegevensstroom, met inbegrip van uw eindpunt URL terug, duidelijk als `inletUrl`. Zie de [Webhaak instellen](../../../ui/create/marketing-automation/chatlio-webhook.md#get-streaming-endpoint-url) pagina om de vereiste waarde te verkrijgen.
