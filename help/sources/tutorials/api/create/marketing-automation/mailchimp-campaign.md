...
# Een dataflow maken voor [!DNL MailChimp Campaign] met behulp van de Flow Service API

De volgende zelfstudie begeleidt u door de stappen om een bronverbinding en een gegevensstroom tot stand te brengen om [!DNL MailChimp Campaign] gegevens aan Platform te brengen gebruikend [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Nadat u de [!DNL MailChimp]-verificatiereferenties hebt opgehaald, kunt u het maken van een gegevensstroom starten om [!DNL MailChimp Campaign]-gegevens naar het Platform te brengen. De eerste stap bij het maken van een gegevensstroom is het maken van een basisverbinding.
...
Het volgende verzoek leidt tot een basisverbinding voor [!DNL MailChimp]:
...
>[!TIP]
>
>Om het toegelaten formaat-type voor `{SOURCE_PARAMS}` terug te winnen, moet u het volledige `campaignId` koord in base64 coderen. `{"campaignId": "c66a200cda"}` gecodeerd in base64 komt bijvoorbeeld overeen met `eyJjYW1wYWlnbklkIjoiYzY2YTIwMGNkYSJ9`.
...
| `params.campaignId` | De [!DNL MailChimp] campagne-id identificeert een specifieke [!DNL MailChimp]-campagne, waarmee u vervolgens e-mailberichten naar uw lijsten/publiek kunt verzenden. |
...
Met het volgende verzoek wordt een doelverbinding voor [!DNL MailChimp] gemaakt:
...
| Parameter | Beschrijving |
| --------- | ----------- |
| `{BASE_CONNECTION_ID}` | De unieke `id`-waarde voor de basisverbinding die u wilt verwijderen. |
