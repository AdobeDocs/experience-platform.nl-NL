---
title: Een bronverbinding en gegevensstroom maken voor SAP Hybris met behulp van de Flow Service API
description: Leer hoe u een bronverbinding en gegevensstroom maakt om SAP Hybris-gegevens naar het Experience Platform te brengen met behulp van de Flow Service API.
badge: Beta
hide: true
hidefromtoc: true
...
# Een bronverbinding en gegevensstroom maken voor [!DNL SAP Hybris] de Flow Service API gebruiken
...
De volgende zelfstudie begeleidt u door de stappen om een [!DNL SAP Hybris] bronverbinding en te halen gegevensstroom [[!DNL SAP] Abonnementsfacturering](https://www.sap.com/products/financial-management/subscription-billing.html) contactpersonen en klantgegevens naar Adobe Experience Platform met behulp van de [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
De volgende secties bevatten aanvullende informatie die u nodig hebt om verbinding te kunnen maken met [!DNL SAP Hybris] met de [!DNL Flow Service] API.
...
Voor meer informatie over deze referenties raadpleegt u de [[!DNL SAP Hybris] documentatie](https://help.sap.com/docs/CLOUD_TO_CASH_OD/987aec876092428f88162e438acf80d6/c5fcaf96daff4c7a8520188e4d8a1843.html).

## Verbinden [!DNL SAP Hybris] naar Platform met de [!DNL Flow Service] API

Hieronder worden de stappen beschreven die u moet uitvoeren om uw [!DNL SAP Hybris] bron, creeer een bronverbinding, en creeer een gegevensstroom om uw rekeningen en contactgegevens aan Experience Platform te brengen.
...
Om een identiteitskaart van de basisverbinding te creëren, doe een verzoek van de POST aan `/connections` eindpunt terwijl het verstrekken van uw [!DNL SAP Hybris] verificatiereferenties als onderdeel van de aanvraaginstantie.
...
Met de volgende aanvraag wordt een basisverbinding gemaakt voor [!DNL SAP Hybris]:
...
| `{SOURCE_PARAMS}` | Bepaalt parameters voor het brondossier u aan Platform wilt brengen. Het geaccepteerde indelingstype ophalen voor `{SOURCE_PARAMS}`, moet u het volledige koord in base64 coderen. <br> [!DNL SAP Hybris] ondersteunt meerdere API&#39;s. Afhankelijk van het objecttype dat u gebruikt, geeft u een van de volgende waarden door: <ul><li>`customers`</li><li>`contacts`</li></ul> |

De [!DNL SAP Hybris] bron ondersteunt meerdere API&#39;s. Afhankelijk van het objecttype dat u gebruikt voor het verzenden van de aanvraag, ziet u als volgt:
...
Voor [!DNL SAP Hybris] De klant-API heeft de waarde voor `{SOURCE_PARAMS}` wordt doorgegeven als `{"object_type":"customers"}`. Wanneer gecodeerd in base64, komt deze overeen met `eyJvYmplY3RfdHlwZSI6ImN1c3RvbWVycyJ9` zoals hieronder weergegeven.
...
Voor [!DNL SAP Hybris] Contacteert API de waarde voor `{SOURCE_PARAMS}` wordt doorgegeven als `{"object_type":"contacts"}`. Wanneer gecodeerd in base64, komt deze overeen met `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` zoals hieronder weergegeven.
...
Met de volgende aanvraag wordt een bronverbinding gemaakt voor [!DNL SAP Hybris] klantgegevens:
...
| `baseConnectionId` | De basis verbindings-id van [!DNL SAP Hybris]. Deze id is gegenereerd in een eerdere stap. |
| `connectionSpec.id` | De verbindingsspecificatie-id die overeenkomt met uw bron. |
| `data.format` | Het formaat van de [!DNL SAP Hybris] gegevens die u wilt invoeren. Momenteel is de enige ondersteunde gegevensindeling `json`. |
| `object_type` | [!DNL SAP Hybris] ondersteunt meerdere API&#39;s. Voor klanten-API `object_type` parameter moet worden ingesteld op `customers`. |
...
Met de volgende aanvraag wordt een bronverbinding gemaakt voor [!DNL SAP Hybris] contactgegevens:
...
| `connectionSpec.id` | De verbindingsspecificatie-id die overeenkomt met uw bron. |
| `data.format` | Het formaat van de [!DNL SAP Hybris] gegevens die u wilt invoeren. Momenteel is de enige ondersteunde gegevensindeling `json`. |
| `object_type` | [!DNL SAP Hybris] ondersteunt meerdere API&#39;s. Voor contact-API `object_type` parameter moet worden ingesteld op `contacts`. |
...
Met de volgende aanvraag wordt een doelverbinding gemaakt voor [!DNL SAP Hybris]:
...
| `data.format` | Het formaat van de [!DNL SAP Hybris] gegevens die u wilt invoeren. |
...
Met de volgende aanvraag wordt een toewijzing gemaakt voor [!DNL SAP Hybris] Klanten-API-gegevens
...
Met de volgende aanvraag wordt een toewijzing gemaakt voor [!DNL SAP Hybris] Contactpersonen-API-gegevens
...
De laatste stap op weg naar de [!DNL SAP Hybris] aan Platform is een gegevensstroom tot stand te brengen. Momenteel zijn de volgende vereiste waarden voorbereid:
