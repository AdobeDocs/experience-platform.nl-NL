...
De volgende zelfstudie begeleidt u door de stappen om een [!DNL Pinterest Ads] bronverbinding en gegevensstroom die moet worden opgehaald [[!DNL Pinterest Ads]](https://ads.pinterest.com/) gegevens naar Adobe Experience Platform met behulp van de [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
De volgende secties bevatten aanvullende informatie die u nodig hebt om verbinding te kunnen maken met [!DNL Pinterest Ads] met de [!DNL Flow Service] API.
...
* De [!DNL Pinterest] `accessToken`.
* De [!DNL Pinterest] `adAccountId`.
* Eén van [!DNL Pinterest] `campaign`, `adGroup` of `ad` ID&#39;s naar wens.
...
## Verbinden [!DNL Pinterest Ads] naar Platform met de [!DNL Flow Service] API {#connect-platform-to-flow-api}

Hieronder worden de stappen beschreven die moeten worden uitgevoerd om verbinding te maken [!DNL Pinterest Ads] naar Experience Platform.
...
Om een identiteitskaart van de basisverbinding te creëren, doe een verzoek van de POST aan `/connections` als u uw [!DNL Pinterest Ads] verificatiegegevens als onderdeel van de aanvraaginstantie.
...
Met de volgende aanvraag wordt een basisverbinding gemaakt voor [!DNL Pinterest Ads]:
...
| `auth.params.accessToken` | Bevat de [!DNL Pinterest] Tokenwaarde van de toegang wordt vereist om uw bron voor authentiek te verklaren. |
...
| `{SOURCE_PARAMS}` | Bepaalt parameters voor het brondossier u aan Platform wilt brengen. Het geaccepteerde indelingstype ophalen voor `{SOURCE_PARAMS}`, moet u het gehele `{"ad_account_id":"{PINTEREST_AD_ACCOUNT_ID}","object_ids":"{COMMA_SEPERATED_OBJECT_IDS}","object_type":"{OBJECT_TYPE}}"}` string in base64. |

[!DNL Pinterest Ads] ondersteunt meerdere [!DNL Pinterest] Analytics API-eindpunten. Afhankelijk van het objecttype dat u gebruikt voor het verzenden van de aanvraag, ziet u als volgt:
...
Voor [!DNL Pinterest Ads]Als u de API voor Campagne Analytics gebruikt, moet u de waarde voor `{SOURCE_PARAMS}` wordt doorgegeven als `{"ad_account_id":"123456789000","object_ids":"000123456789","object_type":"campaigns"}`. Wanneer gecodeerd in base64, komt deze overeen met `YHsiYWRfYWNjb3VudF9pZCI6IjEyMzQ1Njc4OTAwMCIsIm9iamVjdF9pZHMiOiIwMDAxMjM0NTY3ODkiLCJvYmplY3RfdHlwZSI6ImNhbXBhaWducyJ9` zoals hieronder weergegeven.
...
Voor [!DNL Pinterest Ads]Als u de API voor Advertentiegroepen Analytics gebruikt, moet u de waarde voor `{SOURCE_PARAMS}` wordt doorgegeven als `{"ad_account_id":"123456789000","object_ids":"000123456789,100123456789","object_type":"ad_groups"}`. Wanneer gecodeerd in base64 staat deze gelijk aan `eyJhZF9hY2NvdW50X2lkIjoiMTIzNDU2Nzg5MDAwIiwib2JqZWN0X2lkcyI6IjAwMDEyMzQ1Njc4OSwxMDAxMjM0NTY3ODkiLCJvYmplY3RfdHlwZSI6ImFkX2dyb3VwcyJ9` zoals hieronder weergegeven.
...
Voor [!DNL Pinterest Ads]Als u de API voor AdvertentiAnalyse gebruikt, moet u de waarde voor `{SOURCE_PARAMS}` wordt doorgegeven als `{"ad_account_id":"123456789000","object_ids":"687247811001,687247811002,687247815005,687247834765","object_type":"ads"}`. Wanneer gecodeerd in base64 staat deze gelijk aan `eyJhZF9hY2NvdW50X2lkIjoiMTIzNDU2Nzg5MDAwIiwib2JqZWN0X2lkcyI6IjY4NzI0NzgxMTAwMSw2ODcyNDc4MTEwMDIsNjg3MjQ3ODE1MDA1LDY4NzI0NzgzNDc2NSIsIm9iamVjdF90eXBlIjoiYWRzIn0=` zoals hieronder weergegeven.
...
Een geslaagde reactie retourneert de gegevensstructuur van het corresponderende [!DNL Pinterest Ads] API die u riep.
...
U kunt een bronverbinding tot stand brengen door een verzoek van de POST aan [!DNL Flow Service] API. Een bronverbinding bestaat uit een basis verbindings-id, een pad naar het brongegevensbestand en een verbindingsspecificatie-id.
...
De [!DNL Pinterest Ads] bron ondersteunt meerdere [!DNL Pinterest] Analytics API-eindpunten. Afhankelijk van het objecttype dat u gebruikt, wordt een bronverbinding gemaakt met de volgende aanvraag:
...
| `data.format` | Het formaat van de [!DNL Pinterest Ads] gegevens die u wilt invoeren. Momenteel is de enige ondersteunde gegevensindeling `json`. |
| `params.ad_account_id` | De [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | Als de [!DNL Pinterest] Het API-eindpunt van Campagne Analytics is vereist. De waarde is `campaigns`. |
...
| `data.format` | Het formaat van de [!DNL Pinterest Ads] gegevens die u wilt invoeren. Momenteel is de enige ondersteunde gegevensindeling `json`. |
| `params.ad_account_id` | De [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | Als de [!DNL Pinterest] Het API-eindpunt voor analyse van groepen is vereist. De waarde is `ad_groups`. |
| `params.object_ids` | De door komma&#39;s gescheiden lijst met [!DNL Pinterest] Groepen-id&#39;s toevoegen. |
...
| `data.format` | Het formaat van de [!DNL Pinterest Ads] gegevens die u wilt invoeren. Momenteel is de enige ondersteunde gegevensindeling `json`. |
| `params.ad_account_id` | De [!DNL Pinterest] `Ad account ID`. |
| `params.object_type` | Als de [!DNL Pinterest] Het API-eindpunt voor Adanalyse is vereist, de waarde is `ads`. |
| `params.object_ids` | De door komma&#39;s gescheiden lijst met [!DNL Pinterest] ID&#39;s toevoegen. |
...
Met de volgende aanvraag wordt een doelverbinding gemaakt voor [!DNL Pinterest Ads]:
...
| `data.format` | Het formaat van de [!DNL Pinterest Ads] gegevens die u naar Platform wilt brengen. |
...
De laatste stap op weg naar het verzamelen van gegevens van [!DNL Pinterest Ads] aan Platform moet een gegevensstroom creëren. Momenteel zijn de volgende vereiste waarden voorbereid:
