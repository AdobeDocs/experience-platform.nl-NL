...
Een dataflow is een geplande taak die gegevens van een bron aan een dataset in Adobe Experience Platform terugwint en opneemt. Deze zelfstudie biedt stappen voor het maken van een gegevensstroom voor een databasebron met behulp van de interface van het Platform.
...
Nadat u uw databasebronaccount hebt gemaakt, kunt u **[!UICONTROL Add data]** stap verschijnt, die een interface voor u verstrekken om de de lijsthiërarchie van uw rekening van de gegevensbestandbron te onderzoeken.
...
Door deze zelfstudie te volgen, hebt u met succes een gegevensstroom gecreeerd om gegevens van uw gegevensbestandbron aan Platform te brengen. Binnenkomende gegevens kunnen nu door downstreamgebruikers worden gebruikt [!DNL Platform] diensten zoals [!DNL Real-time Customer Profile] en [!DNL Data Science Workspace]. Raadpleeg de volgende documenten voor meer informatie:
