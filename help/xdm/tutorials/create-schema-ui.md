...
Alle gegevens die in het veld `personalEmail.address` worden ingevoerd, worden nu gebruikt om die persoon te identificeren en één weergave van die klant samen te voegen. Raadpleeg de documentatie van [!DNL Experience Platform] voor meer informatie over het werken met identiteiten in [[!DNL Identity Service]](../../identity-service/home.md) .
...
>[!NOTE]
>
>Wanneer het werken met middelen XDM, zijn de acties beschikbaar bij zowel de inventarislijst (rijmenu) als de mening van het middeldetail (**[!UICONTROL More]**). Om tot de volledige reeks acties toegang te hebben, met inbegrip van **Schrapping**, **de structuur van JSON van het Exemplaar**, en **voeg aan pakket** toe, moet u een douane (huurder-bepaald) middel selecteren. Standaard (door Adobe geleverd) bronnen hebben beperkte acties. Voor een volledig overzicht van acties, beperkingen, en toestemmingen, zie [&#x200B; schema&#39;s, klassen, gebiedsgroepen, en gegevenstypes beheren: acties en schrapping &#x200B;](../ui/explore.md#xdm-resource-actions).
