...
Mixins zijn herbruikbare componenten die een of meer velden definiëren die een bepaald concept vertegenwoordigen, zoals een individuele persoon, een mailingadres of een webbrowseromgeving. Mixins zijn bedoeld om te worden opgenomen als onderdeel van een schema dat een compatibele klasse implementeert, afhankelijk van het gedrag van de gegevens die ze vertegenwoordigen (record- of tijdreeks). Het `/mixins` eindpunt in [!DNL Schema Registry] API staat u toe om combinaties binnen uw ervaringstoepassing programmatically te beheren.
...
U kunt alle mixen onder de `global` container weergeven door een verzoek van de GET in te dienen bij `tenant` respectievelijk `/global/mixins` `/tenant/mixins`.
...
De antwoordindeling is afhankelijk van de `Accept` header die in de aanvraag wordt verzonden. De volgende `Accept` kopteksten zijn beschikbaar voor aanbiedingsmengsels:
...
| `application/vnd.adobe.xed+json` | Retourneert de volledige JSON-mix voor elke bron, met origineel `$ref` en `allOf` opgenomen. (Limiet: 300) |

**Antwoord**

In het bovenstaande verzoek werd de `application/vnd.adobe.xed-id+json` header gebruikt. Daarom bevat het antwoord alleen de kenmerken `Accept` , `title`, `$id`en `meta:altId``version` kenmerken voor elke mix. Als u de andere `Accept` header (`application/vnd.adobe.xed+json`) gebruikt, worden alle kenmerken van elke mix geretourneerd. Selecteer de juiste `Accept` koptekst, afhankelijk van de informatie die u in uw reactie nodig hebt.
...
| `{MIXIN_ID}` | De `meta:altId` of URL-gecodeerde `$id` mix die u wilt opzoeken. |

**Verzoek**

Met het volgende verzoek wordt een mix opgehaald op basis van de `meta:altId` waarde die in het pad is opgegeven.
...
U kunt een aangepaste mix onder de `tenant` container definiëren door een aanvraag voor een POST in te dienen.
...
Wanneer u een nieuwe mix definieert, moet deze een `meta:intendedToExtend` kenmerk bevatten met een overzicht `$id` van de klassen waarmee de mix compatibel is. In dit voorbeeld is de mix compatibel met een `Property` klasse die eerder is gedefinieerd. Aangepaste velden moeten onder `_{TENANT_ID}` (zoals in het voorbeeld) worden genest om conflicten te voorkomen met vergelijkbare velden die door klassen en andere mixen worden verschaft.

>[!NOTE]
>
>Zie de handleiding voor [veldbeperkingen voor meer informatie over het definiëren van verschillende veldtypen die u in uw mix wilt opnemen](../schema/field-constraints.md#define-fields).
...
Het uitvoeren van een verzoek van de GET om van alle mengen [in de huurderscontainer een](#list) lijst te maken zou nu de mengeling van de Details van het Bezit omvatten, of u kunt een raadpleging (GET) verzoek [uitvoeren gebruikend URL-Gecodeerde](#lookup) `$id` URI om de nieuwe mengeling direct te bekijken.

## Een mix bijwerken {#put}

U kunt een volledige mix door een verrichting van de PUT vervangen, hoofdzakelijk herschrijvend het middel. Wanneer het bijwerken van een mengeling door een verzoek van de PUT, moet het lichaam alle gebieden omvatten die worden vereist wanneer [het creëren van een nieuwe mengeling](#create) in een verzoek van de POST.

>[!NOTE]
>
>Als u slechts een deel van een mengeling wilt bijwerken in plaats van het volledig te vervangen, zie de sectie over het [bijwerken van een gedeelte van een mengeling](#patch).
...
| Parameter | Beschrijving |
| --- | --- |
| `{MIXIN_ID}` | De `meta:altId` of URL-gecodeerde `$id` mix die u opnieuw wilt schrijven. |

**Verzoek**

In het volgende verzoek wordt een bestaande mix opnieuw geschreven en wordt een nieuw `propertyCountry` veld toegevoegd.
...
U kunt een gedeelte van een mix bijwerken door een verzoek van PATCH te gebruiken. De [!DNL Schema Registry] functie ondersteunt alle standaard JSON-patchbewerkingen, inclusief `add`, `remove`en `replace`. Zie de handleiding [voor](../../landing/api-fundamentals.md#json-patch)API-basisbeginselen voor meer informatie over JSON Patch.

>[!NOTE]
>
>Als u een volledige bron wilt vervangen door nieuwe waarden in plaats van afzonderlijke velden bij te werken, raadpleegt u de sectie over het [vervangen van een combinatie met een PUT-bewerking](#put).
...
| Parameter | Beschrijving |
| --- | --- |
| `{MIXIN_ID}` | De URL-gecodeerde `$id` URI of `meta:altId` de mix die u wilt bijwerken. |

**Verzoek**

In de onderstaande voorbeeldaanvraag wordt de status `description` van een bestaande mix bijgewerkt en wordt een nieuw `propertyCity` veld toegevoegd.
...
De reactie toont aan dat beide bewerkingen met succes zijn uitgevoerd. Het `description` is bijgewerkt en `propertyCountry` is toegevoegd onder `definitions`.
...
U kunt de schrapping bevestigen door een [raadpleging (GET) verzoek](#lookup) aan de mixin te proberen. U zult een `Accept` kopbal in het verzoek moeten omvatten, maar zou een status 404 van HTTP (niet Gevonden) moeten ontvangen omdat de mixin uit de Registratie van het Schema is verwijderd.