...
Alvorens deze gids te lezen, te herzien gelieve [XDM systeemoverzicht](../home.md) voor een inleiding op hoog niveau aan XDM en zijn rol binnen Experience Platform.
...
Zodra u de gegevensbronnen hebt bepaald u om in [!DNL Platform] wilt brengen, creeer een ERD op hoog niveau helpen het proces begeleiden om uw gegevens aan schema&#39;s toe te wijzen XDM.

In het onderstaande voorbeeld ziet u een vereenvoudigde ERD voor een bedrijf dat gegevens wil overbrengen naar [!DNL Platform]. Het diagram benadrukt de essentiële entiteiten die in klassen XDM, met inbegrip van klantenrekeningen, hotels, adressen, en verscheidene gemeenschappelijke e-commercegebeurtenissen zouden moeten worden gesorteerd.

![](../images/best-practices/erd.png)

## Entiteiten sorteren in profiel-, zoekopdracht- en gebeurteniscategorieën

Zodra u een ERD hebt gecreeerd om de essentiële entiteiten te identificeren u in [!DNL Platform] zou willen brengen, moeten deze entiteiten in profiel, raadpleging, en gebeurteniscategorieën worden gesorteerd:

| Categorie | Beschrijving |
| --- | --- |
| Profielentiteiten | Profielentiteiten vertegenwoordigen kenmerken die betrekking hebben op een individuele persoon, doorgaans een klant. Entiteiten die onder deze categorie vallen, moeten worden vertegenwoordigd door schema&#39;s op basis van de **[!DNL XDM Individual Profile]-klasse**. |
...
Als u wilt analyseren hoe bepaalde kenmerken binnen een entiteit in de loop der tijd veranderen, is het waarschijnlijk een gebeurtenisentiteit. Bijvoorbeeld, kan het toevoegen van productpunten aan een karretje als toe:voegen-aan-kartgebeurtenissen in [!DNL Platform] worden gevolgd:
...
>[!NOTE]
>
>Aangezien er geen universele aanpak bestaat om alle gevallen van gebruik te kunnen gebruiken, is het belangrijk om bij de indeling van entiteiten op basis van kardinaliteit rekening te houden met de voor- en nadelen van elke situatie. Zie [volgende sectie](#pros-and-cons) voor meer informatie.
...
In Experience Platform, worden de gebieden XDM duidelijk als identiteiten gebruikt om informatie over individuele klanten te verbinden die uit veelvoudige gegevensbronnen komen. Hoewel een schema veelvoudige gebieden kan hebben duidelijk als identiteiten, moet één enkele primaire identiteit worden bepaald opdat het schema voor gebruik in [!DNL Real-time Customer Profile] wordt toegelaten. Zie de sectie over [identiteitsgebieden](./composition.md#identity) in de grondbeginselen van schemacompositie voor meer gedetailleerde informatie over het gebruiksgeval van deze gebieden.
...
Zodra u klaar bent, zie de zelfstudie op [creërend een schema in UI](../tutorials/create-schema-ui.md) voor geleidelijke instructies op hoe te om een schema tot stand te brengen, de aangewezen klasse voor de entiteit toe te wijzen, en gebieden toe te voegen om uw gegevens aan in kaart te brengen.