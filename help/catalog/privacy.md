...
| `xdm:property` | Of &quot;xdm :id&quot;of &quot;xdm :code&quot;, afhankelijk van namespace die onder `xdm:namespace` wordt gebruikt. |
...
Bij het maken van taakaanvragen in de API moeten alle `userIDs` die worden geleverd, een specifieke `namespace` en `type` gebruiken, afhankelijk van de gegevensopslag waarop ze van toepassing zijn. Een geldige naamruimte die door Identity Service wordt herkend, moet worden opgegeven voor de naamruimtewaarde. Gebruik `standard` voor standaardnaamruimten en gebruik `custom` voor aangepaste naamruimten.

IDs voor het gegevensmeer moet `unregistered` voor hun `type` waarde, en a `namespace` waarde gebruiken die één [ privacyetiketten ](#privacy-labels) aanpast die aan toepasselijke datasets zijn toegevoegd.
