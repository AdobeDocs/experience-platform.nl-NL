...
Bij het maken van taakaanvragen in de API moeten alle id&#39;s in `userIDs` een specifieke `namespace` en `type` gebruiken. Een geldige [&#x200B; identiteit namespace &#x200B;](#namespaces) die door de Dienst van de Identiteit wordt erkend moet voor de namespace waarde worden verstrekt. Gebruik `standard` voor standaardnaamruimten en gebruik `custom` voor aangepaste naamruimten.
