...
## Versie 2.32.0 - 23 maart 2026

- De gedeelde kernnut worden nu gepubliceerd als standalone npm pakket ([@adobe/alloy-core ](https://www.npmjs.com/package/@adobe/alloy-core)) voor gebruik door uitbreidingen en integratie.
- Bevat nu de IANA-tijdzone in het XDM-veld `xdm.placeContext.ianaTimezone` wanneer `placeContext` is opgenomen in de configuratievariabele [`context`](/help/collection/js/commands/configure/context.md) .
- Merk concierge: Probleem met sessie-id verholpen waarbij [`stickyConversationSession`](/help/collection/js/commands/configure/conversation.md) is uitgeschakeld.

## Versie 2.31.1 - 11 februari 2026

- Probleem verholpen waarbij de Web SDK vastloopt als de URL meerdere aan reclame gerelateerde `s_kwcid` of `ef_id` -parameters bevat.
- Probleem verholpen waarbij Advertising-gegevens werden verzonden en cookies werden gemaakt voordat toestemming werd gegeven.
- Probleem opgelost in Safari waarbij Brand Concierge-streams niet correct werden geparseerd.
...
- Toegevoegde steun voor het verzoeken van [ identiteitskaart van de KERN ](/help/collection/identity/overview.md#core-id-and-third-party-identity) in het [ getIdentity ](commands/getidentity.md) bevel.
...
- U kunt nu nauwkeuriger persoonlijke ervaringen bieden door gebruikers-id&#39;s te delen tussen mobiele apps en mobiele webinhoud, en tussen domeinen. Zie [ Identiteit in de Inzameling van Gegevens ](../identity/overview.md) om meer te leren.
- U kunt nu een array met voorstellingen van [!DNL Adobe Target] renderen of uitvoeren in toepassingen van één pagina, zonder de analytische meetgegevens te verhogen. Dit vermindert rapportagefouten en vergroot de nauwkeurigheid van de analyse.
