...
**Toegangstoets-id**: Een toegangs belangrijkste identiteitskaart is een uniek herkenningsteken dat met een wordt geassocieerd [!DNL Amazon] S3 geheime toegangssleutel. De toegangs belangrijkste identiteitskaart en de geheime toegangssleutel worden gebruikt samen om te ondertekenen [!DNL Amazon Web Services] (AWS) verzoeken.
...
**Activeren**: Activeren is de actie die door een gebruiker wordt ondernomen om een segment of profielen aan een bestemming zoals in kaart te brengen [!DNL Oracle Eloqua], [!DNL Google], of [!DNL Salesforce Marketing Cloud].
...
**Afbeelding**: In [!DNL Offer Decisioning], wordt de aftopping (ook wel &quot;frequency capping&quot; genoemd) gebruikt in beslissingsregels om te bepalen hoe vaak een aanbieding wordt gepresenteerd. Er zijn twee typen uiteinden: hoe vaak een aanbieding over het gecombineerde doelpubliek kan worden voorgesteld (genoemd een &quot;Globale Uiteinde&quot;), en hoeveel keer een aanbieding aan het zelfde eind kan worden voorgesteld (genoemd een &quot;Uiteinde van het Profiel&quot;).

**Catalogus**: In de context van bronnen en bestemmingen, is een catalogus een galerij met beschikbare verbindingen aan Adobe toepassingen en derdetechnologieën. Niet verward met [!DNL Catalog Service].
...
**Opdrachtregelinterface**: Een opdrachtregelinterface is een tekstgebaseerd gereedschap waarmee u verbinding kunt maken [!DNL Query Service] voor uitvoering van onbewerkte query.
...
**Customer AI**: AI van de klant is een intelligente dienst die door Adobe Sensei wordt aangedreven die klantenprofielen met op AI-Gebaseerde eigenschappen verrijkt en klantensegmentatie en gerichte inspanningen toelaat.
...
**Dataset-id**: Een door Adobe gegenereerde id voor een opgenomen gegevensset.

**Uitvoer gegevensset**: Datasetuitvoer biedt een mechanisme om te bepalen welke optie &#39;Tabel maken als selectie&#39; wordt gebruikt voor een bepaalde gegevensset [!DNL Query Service] uitvoeren.
...
**Delta, kolom**: In een deltabkolom kunt u een brongegevensveld selecteren dat een tijdstempel voor incrementele opname vertegenwoordigt.
...
**Descriptor**: In het Model van Gegevens van de Ervaring (XDM), is een beschrijver een extra reeks van op schema betrekking hebbende meta-gegevens die een specifiek gedrag voor een gebied beschrijft. De beschrijvers kunnen door Experience Platform worden gebruikt om voorgenomen schemagedrag zoals het verband tussen twee schema&#39;s te begrijpen.
...
**Regels voor directe call**: In de context van markeringen, is een direct-vraagregel een regel die uitvoert wanneer het direct van de pagina wordt geroepen, die gebeurtenis-opsporing en raadplegingssystemen omzeilt.
...
**Subsidiabiliteitsregels**: In [!DNL Offer Decisioning], worden de subsidiabiliteitsregels toegepast op een profiel dat betrekking heeft op kalender-, schema- en plafondbeperkingen.

**Marketingactie e-mail**: A marketing action that uses data in email targeting campagnes.
...
**Experience Event**: Een Experience Event vertegenwoordigt een momentopname van het systeem wanneer een interactie of een gebeurtenis met betrekking tot een klantenervaring plaatsvindt. De Gebeurtenissen van de ervaring zijn onveranderlijke feitenverslagen van wat voorkwam en vertegenwoordigen wat gebeurde zonder samenvoeging of interpretatie. In het Model van de Gegevens van de Ervaring (XDM), wordt dit concept gevangen door [!DNL XDM ExperienceEvent] klasse.
...
**Frequentie**: In [!DNL Query Service], bepaalt de frequentie hoe vaak een terugkerende geplande query wordt uitgevoerd.
...
**Identiteitsstitatie**: Identiteitsstitching is het proces om gegevensfragmenten te identificeren en hen samen te binden om een volledig profielverslag te vormen.
...
**[!DNL JupyterLab]**: Een open-bron, web-based interface voor Project [!DNL Jupyter] die in de gebruikersinterface van het Platform is geïntegreerd.
...
**Werkruimte Beleid**: Een werkruimte in de gebruikersinterface van het Platform die gegevens toelaat om de etiketten en het beleid van het gegevensgebruik voor uw organisatie te bekijken en te beheren.
...
**Prioriteit**: In [!DNL Offer Decisioning], wordt prioriteit gebruikt om aanbiedingen te rangschikken die aan alle beperkingen voldoen, zoals geschiktheid, kalender en aftopping.
...
**Query-editor**: De Redacteur van de vraag is een hulpmiddel om SQL verklaringen te schrijven, te bevestigen en voor te leggen in [!DNL Query Service].
...
**Resource**: In de context van markeringen, is een middel een generische termijn die naar opties verwijst de gebruiker kan binnen het cliëntmilieu vormen, met inbegrip van uitbreidingen, gegevenselementen, en regels.
...
**component Rule**: In de context van labels zijn regelcomponenten de gebeurtenissen, voorwaarden en handelingen die een regel vormen.

**Runtime**: Runtime specificeert een runtime milieu voor een machine-leert recept. [!DNL Python], R, [!DNL Spark]Met de runtimes PySpark en Tensorflow kunt u een URL invoeren naar een Docker-afbeelding voor een verwerkingsbron.
...
**Schema-bibliotheek**: De schemabibliotheek bevat industrie-standaard XDM middelen die door Adobe, evenals douanemiddelen beschikbaar worden gemaakt die door uw organisatie worden bepaald.

**Schema-register**: Het register van het Schema verstrekt een gebruikersinterface en RESTful API die wordt gebruikt om alle schema-verwante middelen in de Bibliotheek van het Schema te bekijken en te beheren.

**Geheime toegangstoets**: Een geheime toegangssleutel is een [!DNL Amazon] S3 sleutel die samen met toegangs belangrijkste identiteitskaart wordt gebruikt om AWS verzoeken te ondertekenen.
...
**Segment Builder**: De [!DNL Segment Builder] is een visuele ontwikkelomgeving die wordt gebruikt om segmentdefinities te bouwen. Het dient als gemeenschappelijke component van alle toepassingen gebruikend de Dienst van de Segmentatie van het Experience Platform.
...
**Segmentering**: De segmentatie is het proces om een grote groep klanten, vooruitzichten, of consumenten in kleinere groepen te verdelen die gelijkaardige eigenschappen delen en op gelijkaardige wijze aan specifieke marketing strategieën zullen antwoorden.

**Sensei ML Framework**: Het Kader van Sensei ML is een verenigd machine-leert (ML) kader dat Experience Platform gegevens gebruikt om gegevenswetenschappers aan de ontwikkeling van ML-gedreven inlichtingendiensten op een snellere, scalable, en herbruikbare manier te machtigen.
...
**S1-label voor gegevensgebruik**: An `S1` het gegevensgebruiksetiket wordt gebruikt om gegevens te classificeren die breedte en lengte specificeren die kunnen worden gebruikt om de nauwkeurige plaats van een apparaat te bepalen.
