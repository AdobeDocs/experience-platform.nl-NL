...
**Toegangstoets-id**: Een toegangstoets-id is een unieke id die is gekoppeld aan een [!DNL Amazon] S3 geheime toegangssleutel. De toegangs belangrijkste identiteitskaart en de geheime toegangssleutel worden gebruikt samen om te ondertekenen [!DNL Amazon Web Services] (AWS) verzoeken.
...
**Activeren**: Activeer is de actie die een gebruiker heeft uitgevoerd om een segment of profielen toe te wijzen aan een doel zoals [!DNL Oracle Eloqua], [!DNL Google], of [!DNL Salesforce Marketing Cloud].
...
**Kenmerk samenvoegen**: Wanneer u een samenvoegbeleid definieert met de Real-Time Customer Profile API, wordt de `attributeMerge` Dit object geeft aan op welke manier in het samenvoegbeleid de profielkenmerken voorrang krijgen in geval van gegevensconflicten. Het is gelijk aan het selecteren van een [!UICONTROL Merge method] wanneer het bepalen van een fusiebeleid in het Platform UI.
...
**Business Intelligence-tools**: Business Intelligence (BI)-tools zijn vooral geïntegreerd met [!DNL Experience Platform Query Service]. BI-gereedschappen zijn typen toepassingssoftware die grote hoeveelheden ongestructureerde gegevens van interne en externe systemen verzamelen en verwerken.
...
**Catalogus**: In de context van bronnen en bestemmingen, is een catalogus een galerij met beschikbare verbindingen aan de toepassingen van de Adobe en derdetechnologieën. Niet verward met [!DNL Catalog Service].
...
**Client**: Een client is een extern hulpprogramma of externe toepassing waarmee verbinding wordt gemaakt [!DNL Query Service] via [!DNL PostgreSQL] protocol of HTTP API.
...
**Uitvoer gegevensset**: De uitvoer van de gegevensset biedt een mechanisme om te bepalen welke optie &#39;Tabel maken als selectie&#39; wordt gebruikt voor een bepaalde [!DNL Query Service] uitvoeren.
...
**[!DNL Intelligent Services]**: [!DNL Intelligent Services] zoals [!DNL Attribution AI] en [!DNL Customer AI] zijn machinaal-leren, op kunstmatige intelligentie-gebaseerde modellen die Experience Platform (of toepassingen die bovenop Platform zoals Adobe Real-time Customer Data Platform worden gebouwd) vereisen om te lopen en te werken.
...
**[!DNL JupyterLab]**: Een opensource, webinterface voor project [!DNL Jupyter] dat is geïntegreerd in de interface van het Platform.

**[!DNL Jupyter Notebook]**: Met JupyterLab en Jupyter-laptops kunt u gegevens opschonen en transformeren, numerieke simulaties, statistische modellering, gegevensvisualisatie, machinaal leren en meer uitvoeren op uw Experience Platforms in verschillende talen, zoals Python, Scala en PySpark.
...
**Query-editor**: De Editor van de query is een hulpmiddel voor het schrijven, valideren en verzenden van SQL-instructies in [!DNL Query Service].
...
**Runtime**: Runtime geeft een runtimeomgeving op voor een recept voor machinaal leren. [!DNL Python], R, [!DNL Spark]Met de runtimes PySpark en Tensorflow kunt u een URL invoeren naar een Docker-afbeelding voor een verwerkingsbron.
...
**Geheim toegangssleutel**: Een geheime toegangstoets is een [!DNL Amazon] S3 sleutel die samen met toegangs belangrijkste identiteitskaart wordt gebruikt om AWS verzoeken te ondertekenen.
...
**Afzonderlijk XDM-profiel**: XDM [!DNL Individual Profile] is een op records gebaseerde klasse die een unieke representatie vormt van de kenmerken van zowel geïdentificeerde als gedeeltelijk geïdentificeerde onderwerpen. De profielen die hoogst geïdentificeerd zijn kunnen voor persoonlijke mededelingen of gerichte overeenkomsten worden gebruikt, en kunnen gedetailleerde persoonlijke informatie zoals naam, geslacht, geboortedatum, plaats, en contactinformatie met inbegrip van telefoonaantallen en e-mailadressen bevatten.
