...
## Vervaldatum van gegevens uitbreiden voor een extern publiek {#extend-data-expiration}

>[!NOTE]
>
>Om het volgende eindpunt te gebruiken, moet u `audienceId` van uw extern publiek hebben. U kunt `audienceId` van een succesvolle vraag aan het `GET /external-audiences/operations/{OPERATION_ID}` eindpunt krijgen.

U kunt de gegevensvervaldatum van een extern publiek uitbreiden door een POST- verzoek aan het volgende eindpunt te doen terwijl het verstrekken van publiekID.

De gegevensvervaldatum wordt verlengd met de oorspronkelijke duur die tijdens de inname is ingesteld. Als er geen tijdsduur is opgegeven, wordt een standaardverlenging van 30 dagen toegepast. Wanneer u de gegevensvervaldatum uitbreidt, wordt het publiek opnieuw opgenomen met de gegevens van de laatste geslaagde opname.
...
Met het volgende verzoek wordt de gegevensvervaldatum van het opgegeven externe publiek uitgebreid.

+++ Een voorbeeldverzoek om de gegevensvervaldatum van een extern publiek uit te breiden.
...
Een geslaagde reactie retourneert HTTP status 200 met details over het publiek.

+++ Een voorbeeldreactie bij het verlengen van de gegevensvervaldatum.
