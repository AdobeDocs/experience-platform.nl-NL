...
Helpen begrijpen hoe het gegeven na opname wordt gegroepeerd en verdeeld, gebruik de volgende vraag om het aantal verschillende fysieke verdelingen (`_ACP_BATCHID`) voor elke logische verdeling (`_acp_system_metadata.acp_sourceBatchId`) te tellen.
...
In dit voorbeeld wordt aangenomen dat u een CSV-bestand in Experience Platform hebt ingevoerd en een gegevensset met de naam `drug_checkout_data` hebt gemaakt.
