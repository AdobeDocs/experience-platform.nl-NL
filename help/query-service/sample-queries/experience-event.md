...
Naast standaard SQL-query&#39;s, Adobe Experience Platform [!DNL Query Service] ondersteunt het schrijven van query&#39;s met [!DNL Experience Events]. De Gebeurtenissen van de ervaring worden vertegenwoordigd door de klasse ExperienceEvent van het Gegevensmodel van de Ervaring (XDM), die een onveranderlijke en niet-geaggregeerde momentopname van het systeem vangt wanneer een gebruiker met een website of een dienst interactie heeft, en kan daarom voor tijd-domeinanalyse worden gebruikt.

Meer informatie over XDM en [!DNL Experience Events] kunt u vinden in het dialoogvenster [[!DNL XDM System] overzicht](../../xdm/home.md). Door combineren [!DNL Query Service] with [!DNL Experience Events], kunt u gedragstrends onder uw gebruikers effectief volgen. Het volgende document bevat voorbeelden van query&#39;s die betrekking hebben op [!DNL Experience Events].
...
De tijdstempelkolom in [!DNL Experience Event] datasets is in UTC. In het volgende voorbeeld wordt het `from_utc_timestamp()` gebruiken om de tijdstempel te transformeren van UTC naar EDT. Vervolgens worden de `date_format()` gebruiken om de datum te isoleren van de rest van de tijdstempel.
...
## Een lijst met bezoekers ophalen op basis van het aantal paginaweergaven.

In het volgende voorbeeld wordt een rapport gemaakt met de id&#39;s van de gebruikers die de meeste pagina&#39;s hebben weergegeven.
...
## Bezoekerssessies opnieuw afspelen
...
## Een rolluprapport van een bezoeker weergeven

In het volgende voorbeeld wordt een geaggregeerd rapport met diverse analysewaarden voor een opgegeven gebruiker weergegeven.
...
Voor meer informatie over steekproefvragen die Adobe bepaalde Functies (ADFs) gebruiken, te lezen gelieve de Adobe bepaalde gids van Functies. Lees voor algemene richtlijnen over het uitvoeren van query&#39;s de [gids over vraaguitvoering in de Dienst van de Vraag](../best-practices/writing-queries.md).
