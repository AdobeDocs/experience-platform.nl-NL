...
Als eerste stap moet u in uw systeem een app maken voor [!DNL Adobe Experience Platform] , of moet u Experience Platform op een andere manier registreren. Het doel is een cliënt-identiteitskaart en cliëntgeheim te produceren, die nodig zijn om Experience Platform aan uw bestemming voor authentiek te verklaren.

Als onderdeel van deze configuratie in uw systeem hebt u de [!DNL Adobe Experience Platform] OAuth 2 redirect/callback URLs nodig, die u uit de onderstaande lijst kunt krijgen.
...
| `authorizationUrl` | String | De URL van uw verificatieserver, waar u de gebruiker omleidt naar uw toepassing. |
...
| `clientId` | String | De client-id die door uw systeem aan [!DNL Adobe Experience Platform] wordt toegewezen. |
| `clientSecret` | String | Het clientgeheim dat uw systeem toewijst aan [!DNL Adobe Experience Platform] . |
...
## OAuth 2 met Client Credentials Grant {#oauth2-client-credentials-grant}
...
| `accessTokenRequest.validations.actualValue.value` | String | Gebruik een sjabloontaal voor toegang tot velden in de HTTP-respons. Voor informatie over hoe te om het templating te gebruiken om gebieden aan te passen, zie de [ sectie van de overeenkomsten van de 0} malplaatjemeting {.](#templating-conventions) |
...
| `accessTokenRequest.validations.expectedValue.value` | String | Gebruik een sjabloontaal voor toegang tot velden in de HTTP-respons. Voor informatie over hoe te om het templating te gebruiken om gebieden aan te passen, zie de [ sectie van de overeenkomsten van de 0} malplaatjemeting {.](#templating-conventions) |
