...
| `value` | String | *Vereist.* Deze tekenreeks is de tekenescapeversie van de sjabloon die de HTTP-aanvragen die door Experience Platform worden verzonden opmaakt in de indeling die door uw doel wordt verwacht. <br> voor informatie over hoe te om het malplaatje te schrijven, lees de sectie op [ gebruikend het templating ](message-format.md#using-templating). <br> voor meer informatie over karakter het ontsnappen, zie de [ norm RFC JSON, sectie zeven ](https://tools.ietf.org/html/rfc8259#section-7). <br> Voor een voorbeeld van een eenvoudige transformatie, zie de [ profielattributen ](message-format.md#attributes) transformatie. |
