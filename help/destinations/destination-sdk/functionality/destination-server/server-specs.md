...
De serverspecificaties van de bestemming bepalen het type van bestemmingsplatform dat de gegevens van [!DNL Adobe Experience Platform], en de communicatie parameters tussen Experience Platform en uw bestemming zal ontvangen. Bijvoorbeeld:
...
Wanneer u een doelserver maakt via Destination SDK, kunt u configuratieparameterwaarden definiëren door deze hard te coderen in de configuratie of door gebruik te maken van sjabloonvelden. In sjabloonvelden worden door de gebruiker opgegeven waarden uit de gebruikersinterface van Experience Platform gelezen.
...
### Wanneer moet u hard-gecodeerde en getemplatificeerde velden gebruiken? {#when-to-use-hard-coded-vs-templatized}
...
Met dit type doelserver kunt u gegevens vanuit [!DNL Adobe Experience Platform] exporteren naar uw doel via HTTP-aanvragen. De serverconfiguratie bevat informatie over de server die de berichten ontvangt (de server aan uw kant).
...
Gebruik deze doelserver om bestanden met [!DNL Adobe Experience Platform] -gegevens te exporteren naar uw Amazon S3-opslag.
...
Gebruik deze doelserver om bestanden met [!DNL Adobe Experience Platform] -gegevens te exporteren naar uw [!DNL SFTP] -opslagserver.
...
Gebruik deze doelserver om bestanden met [!DNL Adobe Experience Platform] -gegevens te exporteren naar uw [!DNL Azure Data Lake Storage] -account.
...
Gebruik deze doelserver om bestanden met [!DNL Adobe Experience Platform] -gegevens te exporteren naar uw [!DNL Azure Blob Storage] -container.
...
Gebruik deze doelserver om bestanden met Experience Platform-gegevens te exporteren naar een [[!DNL Data Landing Zone]](../../../catalog/cloud-storage/data-landing-zone.md) -opslag.
...
| `fileBasedDlzDestination.useCase` | String | *Vereiste*. Stel deze in op `"dlz_destination"` . Deze eigenschap identificeert het doel als een [!DNL Data Landing Zone] -doel. Deze eigenschap wordt alleen gebruikt bij het maken van een [!DNL Data Landing Zone] -doel. |

{style="table-layout:auto"}

## [!DNL Google Cloud Storage] doelserver {#gcs-example}

Gebruik deze doelserver om bestanden met Experience Platform-gegevens te exporteren naar uw [!DNL Google Cloud Storage] -account.
