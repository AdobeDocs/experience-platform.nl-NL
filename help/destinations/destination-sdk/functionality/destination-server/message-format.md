...
## Voorwaarden - [!DNL Adobe Experience Platform] concepten {#prerequisites}
...
* **IdentityMap**. Het identiteitsoverzicht vertegenwoordigt een kaart van alle eindgebruikersidentiteiten in [!DNL Adobe Experience Platform]. Verwijs naar `xdm:identityMap` in het [ XDM gebiedswoordenboek ](../../../../xdm/schema/field-dictionary.md).
...
Deze pagina is gericht op de berichtindeling en de profieltransformatie in gegevens die van [!DNL Adobe Experience Platform] naar bestemmingen worden geëxporteerd.

[!DNL Adobe Experience Platform] exporteert gegevens naar een aanzienlijk aantal doelen, in verschillende gegevensindelingen. Voorbeelden van bestemmingstypen zijn advertentieplatforms (Google), sociale netwerken (Facebook) en cloudopslaglocaties (Amazon S3, Azure Event Hubs).
...
* Het XDM-bronschema (1) en het XDM-doelschema (2) in [!DNL Adobe Experience Platform]
...
Om het proces van de profieltransformatie aan te tonen, gebruikt het voorbeeld hieronder drie gemeenschappelijke profielattributen in [!DNL Adobe Experience Platform]: **voornaam**, **achternaam**, en **e-mailadres**.

>[!NOTE]
>
>De klant brengt de attributen van het bronXDM schema aan het partnerXDM schema in [!DNL Adobe Experience Platform] in kaart UI, in de **Afbeelding** stap van [ activeert bestemmingswerkschema ](../../../ui/activate-segment-streaming-destinations.md#mapping).
...
1. Eenvoudige transformatievoorbeelden. Leer hoe het templating werk met eenvoudige transformaties voor [ attributen van het Profiel ](#attributes), [ het lidmaatschap van de Publiek ](#audience-membership), en [ Identiteit ](#identities) gebieden.
...
3. Sjablonen die de aggregatietoets bevatten. Wanneer u [ configureerbare samenvoeging ](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) in de bestemmingsconfiguratie gebruikt, groepeert Experience Platform de profielen die aan uw bestemming worden uitgevoerd die op criteria zoals publieksidentiteitskaart, publieksnamespace, publieksstatus, of identiteitsnamespaces wordt gebaseerd.
...
>[!IMPORTANT]
>
>Voor een lijst van alle beschikbare profielattributen in [!DNL Adobe Experience Platform], zie het [ XDM gebiedswoordenboek ](../../../../xdm/schema/field-dictionary.md).
...
In het onderstaande `json` worden de gegevens weergegeven die uit [!DNL Adobe Experience Platform] zijn geëxporteerd.
...
Als u [ configureerbare samenvoeging ](../../functionality/destination-configuration/aggregation-policy.md#configurable-aggregation) en reeks `includeSegmentId` aan waar gebruikt, worden de profielen in de berichten van HTTP die naar uw bestemming worden uitgevoerd gegroepeerd door publieksidentiteitskaart. Zie hieronder hoe u tot kijksidentiteitskaart en publieksnamespace in het malplaatje kunt toegang hebben.

**Input**

Houd rekening met de vier onderstaande profielen, waarbij:

* de eerste twee maken deel uit van het publiek met de publiek-id `788d8874-8007-4253-92b7-ee6b6c20c6f3` onder de naamruimte `ups`
* het derde profiel maakt deel uit van de doelgroep met de gebruikers-id `8f812592-3f06-416b-bd50-e7831848a31a` onder de naamruimte `CustomerAudienceUpload` .
* het vierde profiel maakt deel uit van beide soorten publiek boven, elk onder hun respectieve naamruimte.
...
Hieronder ziet u hoe `audienceId` en `audienceNamespace` in de sjabloon worden gebruikt om toegang te krijgen tot de gebruikers-id en naamruimte. In dit voorbeeld wordt ervan uitgegaan dat u `audienceId` gebruikt voor publiekslidmaatschap in uw doeltaxonomie. U kunt in plaats daarvan elke andere veldnaam gebruiken, afhankelijk van uw eigen taxonomie.
...
Als de profielen naar uw bestemming worden geëxporteerd, worden deze in twee groepen gesplitst op basis van hun gebruikers-id en naamruimte.
...
| `destination.namespaceSegmentAliases` | Wijs van publiek-id&#39;s in een specifieke naamruimte [!DNL Adobe Experience Platform] toe aan publieksaliassen in het systeem van de partner. | `destination.namespaceSegmentAliases["ups"]["seg-id-1"]` |
| `destination.namespaceSegmentNames` | Wijs van publieksnamen in specifieke [!DNL Adobe Experience Platform] naamruimten toe aan publieksnamen in het systeem van de partner. | `destination.namespaceSegmentNames["ups"]["seg-name-1"]` |
...
| `destination.segmentAliases` | **Vervangen. Vervangen door`destination.namespaceSegmentAliases`** <br><br> Wijs gebruikers-id&#39;s in de naamruimte [!DNL Adobe Experience Platform] toe aan publieksaliassen in het systeem van de partner. | `destination.segmentAliases["seg-id-1"]` |
| `destination.segmentNames` | **Vervangen. Vervangen door`destination.namespaceSegmentNames`** <br><br> Toewijzen van publieksnamen in de naamruimte [!DNL Adobe Experience Platform] aan publieksnamen in het systeem van de partner. | `destination.segmentNames["seg-name-1"]` |
