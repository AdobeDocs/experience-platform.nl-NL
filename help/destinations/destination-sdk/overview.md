---
description: Adobe Experience Platform Destination SDK is een reeks configuratie-API's voor het configureren van bestemmingsintegratiepatronen voor Experience Platform om gebruikers- en profielgegevens te leveren aan uw eindpunt of opslaglocatie, op basis van de gegevens- en verificatieformaten van uw keuze. De configuraties worden opgeslagen in Experience Platform en kunnen via API worden opgehaald voor extra updates.
...
[!DNL Adobe Experience Platform] Destination SDK is een reeks configuratie-API&#39;s waarmee Experience Platform doelintegratiepatronen kan configureren voor het leveren van publiek- en profielgegevens aan uw eindpunt of opslaglocatie, op basis van de gegevens- en verificatieformaten van uw keuze. De configuraties worden opgeslagen in Experience Platform en kunnen via API worden opgehaald voor extra updates.

De documentatie van Destination SDK verstrekt instructies voor u om [!DNL Adobe Experience Platform] Destination SDK te gebruiken om een productieve bestemmingsintegratie met [!DNL Adobe Experience Platform] te vormen, te testen en vrij te geven, en uw bestemming te hebben deel van de steeds groeiende bestemmingscatalogus worden. Met Destination SDK kunt u ook uw eigen aangepaste persoonlijke bestemming maken om gegevens te exporteren die op uw behoeften zijn afgestemd.
...
3. Wordt weergegeven als een productieve doelintegratie met [!DNL Adobe Experience Platform] &amp; Adobe [!DNL Real-Time Customer Data Platform] .
...
Via Destination SDK ondersteunt [!DNL Adobe Experience Platform] realtime (ook wel streaming genoemd) integratie met doelen die een REST API-eindpunt hebben. De real-time integratie met Experience Platform ondersteunt mogelijkheden zoals:
...
* Configureerbare opties voor bestandsindeling waarmee u de indeling van de geëxporteerde bestanden kunt structureren en aan uw vereisten kunt voldoen.
...
Destination SDK-toegang is afhankelijk van uw status als partner of Experience Platform, [!DNL Real-Time CDP] -klant. Zie de onderstaande tabel voor meer informatie.
