...
In dit artikel wordt uitgelegd welke workflow is vereist om het publiek in [!DNL Adobe Experience Platform] te activeren voor op bestanden gebaseerde batchbestemmingen, zoals cloudopslag en marketingdoelen voor e-mail.
...
1. Gebruik de kiezer **[!UICONTROL Frequency]** om de exportfrequentie te selecteren:
   ...
   * **[!UICONTROL Monthly]**: selecteer de begindatum en de volgende exportbewerkingen vinden plaats op die datum van de maand tot de geselecteerde einddatum. Voor maanden met minder dan 30 of 31 dagen vindt de uitvoer plaats op de laatste dag van de maand.
   ...
   > * [&#x200B; de Opslag van de Blob van Azure &#x200B;](../catalog/cloud-storage/azure-blob.md)
...
### Gedetailleerd exportgedrag {#export-behavior}

De geplande uitvoer omvat gegevens van de publieksmomentopname plus om het even welke stijgende profiel of identiteitsveranderingen die tussen momentopname en de uitvoertijd voorkomen. Dit verschilt van [&#x200B; op bestelling de uitvoer &#x200B;](export-file-now.md), die momentopnamegegevens slechts gebruiken.

In de volgende tabel wordt aangegeven hoe de geplande uitvoer verschilt van de uitvoer op aanvraag, met name wat betreft gegevensversheid en het beoogde gebruik:

|  | Geplande uitvoer | Bestand nu exporteren |
|--------|-------------------|-----------------|
| **Gegevensbron** | Momentopname + incrementele wijzigingen | Alleen opname |
| **de attributen van het Profiel** | Huidige waarden tijdens exporteren | Waarden bij opname |

Als de profielen na publieksevaluatie worden bijgewerkt, zal de geplande uitvoer de bijgewerkte attributenwaarden omvatten alhoewel het publiekslidmaatschap op evaluatietijd werd bepaald.

**Voorbeeld**: Een publiek voor &quot;profielen waar retailID ongeldig is&quot;kan profielen met retailID uitvoeren bevolkt als dat gebied *na* evaluatie maar *vóór* de geplande uitvoer werd bijgewerkt.

**Aanbevelingen**

* Vorm a [&#x200B; deduplicatiesleutel &#x200B;](#deduplication-keys) om dubbele verslagen te verhinderen
* Gebruik op bestelling voor exacte op momentopnamen gebaseerde gegevens
* Batchopname afstemmen op evaluatieschema&#39;s om verschillen tot een minimum te beperken

Voor de uitvoer op bestelling, zie de documentatie over [&#x200B; het uitvoeren van dossiers op bestelling &#x200B;](/help/destinations/ui/export-file-now.md#scheduled-vs-ondemand).
...
>[!IMPORTANT]
>
>Configureer altijd een deduplicatiesleutel voor geplande exportbewerkingen. Zonder deduplicatie ziet u mogelijk dubbele rijen of conflicterend segmentlidmaatschap voor hetzelfde profiel, omdat bij geplande exportbewerkingen zowel momentopname als incrementele gegevens worden verwerkt.

Een deduplicatietoets is een door de gebruiker gedefinieerde primaire sleutel die bepaalt hoe profielen worden gededupliceerd. Als er meerdere records voor dezelfde persoon zijn, zorgt deduplicatie ervoor dat alleen de meest recente record wordt geëxporteerd.
...
#### Het kenmerk Publiek-lidmaatschap kan niet worden geselecteerd via de toewijzingsworkflow {#audience-membership-attribute-mapping}
...
#### Naamruimten kunnen momenteel niet worden geselecteerd voor exporteren {#identity-namespaces-export-limitation}
...
>
>Deze stap **[!UICONTROL Select attributes]** wordt nog steeds weergegeven voor de marketingdoelen [!DNL Adobe Campaign] , Oracle Responsys, Oracle Eloqua en Salesforce Marketing Cloud.
...
>[!NOTE]
>
> [!DNL Adobe Experience Platform] vult uw selectie vooraf in met vier aanbevolen, veelgebruikte kenmerken uit uw schema: `person.name.firstName` , `person.name.lastName` , `personalEmail.address` , `segmentMembership.seg_namespace.seg_id.status` .
...
>
> Als het veld `person.name.firstName` bijvoorbeeld bepaalde labels voor gegevensgebruik heeft die conflicteren met de marketingactie van de bestemming, wordt in de revisiestap een schending van het gegevensgebruiksbeleid weergegeven. Voor meer informatie, zie [&#x200B; de Governance van Gegevens in  [!DNL Adobe Experience Platform]](../../rtcdp/privacy/data-governance-overview.md#destinations).
