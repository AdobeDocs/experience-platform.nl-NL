...
>[!AVAILABILITY]
>
>Deze functionaliteit is beschikbaar voor klanten die het [!DNL Real-Time CDP] Prime- of Ultimate-pakket [!DNL Adobe Journey Optimizer] of Customer Journey Analytics hebben aangeschaft. Neem contact op met uw Adobe-vertegenwoordiger voor meer informatie.
...
>
>Zie [&#x200B; het plannen sectie &#x200B;](#scheduling) voor informatie over hoe te om de einddatum van een datasetuitvoer dataflow uit te geven.

Dit artikel verklaart het werkschema dat wordt vereist om [&#x200B; datasets &#x200B;](/help/catalog/datasets/overview.md) van [!DNL Adobe Experience Platform] naar uw aangewezen plaats van de wolkenopslag, zoals [!DNL Amazon S3], plaatsen SFTP, of [!DNL Google Cloud Storage] uit te voeren door Experience Platform UI te gebruiken.
...
De gegevenssets die u kunt exporteren, variëren op basis van de Experience Platform-toepassing ([!DNL Real-Time CDP], [!DNL Adobe Journey Optimizer] ), de laag (Prime of Ultimate) en eventuele invoegtoepassingen die u hebt aangeschaft (bijvoorbeeld Data Distiller).
...
<tbody>
  ...
  <tr>
    ...
    <td>Prime</td>
    <td>Raadpleeg de documentatie bij <a href="https://experienceleague.adobe.com/docs/journey-optimizer/using/data-management/datasets/export-datasets.html?lang=nl-NL#datasets"> [!DNL Adobe Journey Optimizer]</a> .</td>
  </tr>
  <tr>
    <td>Ultimate</td>
    <td>Raadpleeg de documentatie bij <a href="https://experienceleague.adobe.com/docs/journey-optimizer/using/data-management/datasets/export-datasets.html?lang=nl-NL#datasets"> [!DNL Adobe Journey Optimizer]</a> .</td>
...
Gebruik de selectievakjes links van de namen van de gegevenssets om de gegevenssets te selecteren die u naar het doel wilt exporteren en selecteer vervolgens **[!UICONTROL Next]** .
...
>[!NOTE]
>
>Alle hier geselecteerde datasets zullen het zelfde uitvoerprogramma delen. Als u verschillende uitvoerprogramma&#39;s (bijvoorbeeld, stijgende uitvoer voor sommige datasets en eenmalige volledige uitvoer voor anderen) nodig hebt, creeer afzonderlijke dataflows voor elk planningstype.
...
>[!IMPORTANT]
>
>**Programma is op alle datasets in dataflow** van toepassing
>
>Wanneer u vormt of het uitvoerprogramma wijzigt, is het op **van toepassing alle datasets** momenteel die door dataflow worden uitgevoerd u vormt. U kunt geen verschillende programma&#39;s voor individuele datasets binnen zelfde dataflow plaatsen.
>
>Als u verschillende uitvoerprogramma&#39;s voor verschillende datasets nodig hebt, moet u afzonderlijke dataflows (afzonderlijke bestemmingsverbindingen) voor elk planningstype tot stand brengen.
>
>**Voorbeeld:** als u het uitvoeren van Dataset A hebt en u Dataset B met een eenmalig volledig de uitvoerprogramma toevoegt, zal Dataset A ook aan het éénmalige volledige de uitvoerprogramma worden bijgewerkt.
...
>[!WARNING]
>
>Als u het schema hier wijzigt, wordt het exportgedrag voor alle gegevenssets in deze gegevensstroom bijgewerkt. Als dit dataflow veelvoudige datasets bevat, zullen alle hen door deze verandering worden beïnvloed.
...
### Aanbevolen werkwijzen voor het beheer van meerdere gegevenssets {#best-practices-multiple-datasets}

Houd bij het exporteren van meerdere gegevenssets rekening met de volgende aanbevolen procedures:

* **Zelfde planningsvereisten**: De datasets van de groep die het zelfde uitvoerprogramma (frequentie, type) in één enkele dataflow voor gemakkelijker beheer vereisen.
* **Verschillende programmavereisten**: Creeer afzonderlijke dataflows voor datasets die verschillende uitvoerprogramma&#39;s of uitvoertypes (stijgend vs. volledig) vereisen. Dit verzekert elke dataset uitvoer volgens zijn specifieke behoeften.
* **Overzicht alvorens** te wijzigen: Alvorens het programma op een bestaande dataflow te veranderen, herzie welke datasets reeds door die dataflow worden uitgevoerd om onbedoelde veranderingen in hun uitvoergedrag te vermijden.
* **Document uw opstelling**: Houd spoor waarvan datasets waarin dataflows zijn, vooral wanneer het beheren van veelvoudige uitvoerprogramma&#39;s over verschillende bestemmingen.
...
1. Ga naar [&#x200B; UI van Experience Platform &#x200B;](https://experience.adobe.com/platform/) en selecteer **[!UICONTROL Destinations]** van de linkernavigatiebar. Selecteer **[!UICONTROL Browse]** in de bovenste koptekst om de bestaande doelgegevens weer te geven.
...
Raadpleeg de productbeschrijvingsdocumenten om te begrijpen hoeveel gegevens u per jaar voor elke Experience Platform-toepassing mag exporteren. Bijvoorbeeld, kunt u de [!DNL Real-Time CDP] Beschrijving van het Product [&#x200B; hier &#x200B;](https://helpx.adobe.com/nl/legal/product-descriptions/real-time-customer-data-platform-b2c-edition-prime-and-ultimate-packages.html) bekijken.

De gegevensuitvoerrechten voor verschillende aanvragen zijn niet additief. Dit betekent bijvoorbeeld dat als u [!DNL Real-Time CDP] Ultimate en [!DNL Adobe Journey Optimizer] Ultimate aanschaft, de exportrechten voor profielen de hoogste van de twee rechten zijn, zoals in de productbeschrijvingen wordt aangegeven. Uw volumererechten worden berekend door het totale aantal in licentie gegeven profielen te nemen en te vermenigvuldigen met 500 kB voor [!DNL Real-Time CDP] Prime of 700 kB voor [!DNL Real-Time CDP] Ultimate om te bepalen hoeveel gegevensvolume u hebt.
...
* Gegevens met een tijdstempel die ouder is dan 365 dagen, worden niet geëxporteerd voor gegevenssets. Voor meer informatie, bekijk de [&#x200B; gidsen voor de geplande uitvoer van datasets &#x200B;](/help/destinations/guardrails.md#scheduled-dataset-exports)
...
**kunnen wij extra details aan het manifestdossier (namelijk verslagtelling) toevoegen? Zo ja, hoe?**
...
**kan ik verschillende uitvoerprogramma&#39;s voor verschillende datasets in zelfde dataflow plaatsen?**

+++Antwoord
Nr, delen alle datasets binnen één enkele dataflow het zelfde uitvoerprogramma. Als u verschillende uitvoerprogramma&#39;s voor verschillende datasets nodig hebt, moet u afzonderlijke dataflows (bestemmingsverbindingen) voor elk planningstype tot stand brengen. Bijvoorbeeld, als u Dataset A elke dag en Dataset B wilt stapsgewijs exporteren als eenmalige volledige export, moet u twee afzonderlijke dataflows maken.
+++
