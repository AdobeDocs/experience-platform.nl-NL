...
Gebruik het besturingselement **[!UICONTROL Export file now]** om een volledig bestand te exporteren zonder het huidige exportschema van een eerder gepland publiek te onderbreken. Deze exportbewerking wordt uitgevoerd naast de eerder geplande exportbewerkingen en heeft geen invloed op de exportfrequentie van het publiek.
...
Bij uitvoer op aanvraag en bij geplande uitvoer worden verschillende gegevensbronnen gebruikt, wat tot verschillen in geëxporteerde gegevens kan leiden. Raadpleeg de onderstaande tabel om te begrijpen wat er in elk geval wordt geëxporteerd.

|  | Bestand nu exporteren | Geplande uitvoer |
|--------|-----------------|-------------------|
| **Gegevensbron** | Alleen opname | Momentopname + incrementele wijzigingen |
| **de attributen van het Profiel** | Waarden bij opname | Huidige waarden tijdens exporteren |
...
* **[!UICONTROL Export file now]** gebruikt alleen gegevens van geplande momentopnamen. Er worden geen gegevens opgehaald van door API&#39;s geactiveerde exporttaken. Als u de meest recente gegevens wilt exporteren na een door de API geactiveerde exporttaak, wacht u tot de volgende geplande exporttaak wordt uitgevoerd.
