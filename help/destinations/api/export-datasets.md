...
>[!AVAILABILITY]
>
>* Deze functionaliteit is beschikbaar voor klanten die het [!DNL Real-Time CDP] Prime- en Ultimate-pakket [!DNL Adobe Journey Optimizer] of Customer Journey Analytics hebben aangeschaft. Neem contact op met uw Adobe-vertegenwoordiger voor meer informatie.
...
Dit artikel verklaart het werkschema wordt vereist om [!DNL Flow Service API] te gebruiken om [ datasets ](/help/catalog/datasets/overview.md) van [!DNL Adobe Experience Platform] naar uw aangewezen plaats van de wolkenopslag, zoals [!DNL Amazon S3], plaatsen SFTP, of [!DNL Google Cloud Storage] uit te voeren die.
...
De gegevenssets die u kunt exporteren, zijn afhankelijk van de Experience Platform-toepassing ([!DNL Real-Time CDP], [!DNL Adobe Journey Optimizer] ), de laag (Prime of Ultimate) en alle invoegtoepassingen die u hebt aangeschaft (bijvoorbeeld Data Distiller).

Zie de [ lijst op de pagina van het UI leerprogramma ](/help/destinations/ui/export-datasets.md#datasets-to-export) om te begrijpen welke datasets u kunt uitvoeren.
...
Neem nota van de volgende eerste vereisten om datasets uit te voeren:
...
* [[!DNL Experience Platform datasets]](/help/catalog/datasets/overview.md): alle gegevens die correct in [!DNL Adobe Experience Platform] worden ingevoerd, blijven in de [!DNL Data Lake] als gegevenssets behouden. Een dataset is een opslag en beheersconstructie voor een inzameling van gegevens, typisch een lijst, die een schema (kolommen) en gebieden (rijen) bevat. Datasets bevatten ook metagegevens die verschillende aspecten van de gegevens beschrijven die ze opslaan.
...
U hebt deze id&#39;s nodig om verschillende [!DNL Flow Service] -entiteiten samen te stellen. U moet ook verwijzen naar delen van de [!DNL Connection Spec] zelf om bepaalde entiteiten in te stellen, zodat u de [!DNL Connection Spec] from [!DNL Flow Service APIs] kunt ophalen. Zie de voorbeelden hieronder van het terugwinnen van verbindingsspecificaties voor alle bestemmingen in de lijst:
...
+++In aanmerking komende gegevenssets ophalen - Aanvragen
...
Voor informatie over de diverse reactieparameters voor elke teruggekeerde dataset, zie de [ API ontwikkelaarsdocumentatie van Datasets ](https://developer.adobe.com/experience-platform-apis/references/catalog/#tag/Datasets/operation/listDatasets).
...
Daarna, moet u een doelverbinding tot stand brengen die de uitvoerparameters voor uw datasets opslaat. Exportparameters zijn onder andere locatie, bestandsindeling, compressie en andere details. Zie de `targetSpec` -eigenschappen in de verbindingsspecificatie van de bestemming voor meer informatie over de ondersteunde eigenschappen voor elk doeltype. Verwijs in de onderstaande tabbladen naar de eigenschappen `targetSpec` van alle ondersteunde doelen.
...
+++Gegevensset-gegevensstroom maken naar [!DNL Amazon S3] doel - Verzoek
...
+++Gegevensset-gegevensstroom maken naar [!DNL Azure Blob Storage] doel - Verzoek
...
+++Gegevensset-gegevensstroom maken naar [!DNL Azure Data Lake Gen 2(ADLS Gen2)] doel - Verzoek
...
+++Gegevensset-gegevensstroom maken naar [!DNL Data Landing Zone] doel - Verzoek
...
+++Gegevensset-gegevensstroom maken naar [!DNL Google Cloud Storage] doel - Verzoek
...
+++Gegevenssetgegevensstroom maken naar SFTP-bestemming - Verzoek
...
U hebt Experience Platform verbonden met een van uw voorkeursopslaglocaties voor batchcloud en een gegevensstroom ingesteld naar de respectievelijke bestemming voor het exporteren van gegevenssets. Zie de volgende pagina&#39;s voor meer informatie, zoals hoe u bestaande gegevensstromen kunt bewerken met de Flow Service API:
