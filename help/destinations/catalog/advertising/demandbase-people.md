...
Marketers kunnen Adobe [!DNL Real-Time CDP] gebruiken om een personenlijst van eerste-partijcontacten te creëren en het in Demandbase voor geoptimaliseerde en georkestreerde betrokkenheid over zijn vraag-zijplatform (DSP) en andere kanalen zoals LinkedIn te activeren.
...
Voor optimale passende nauwkeurigheid, omvat de volgende facultatieve afbeeldingen in uw activeringsstroom, naast [ verplichte afbeeldingen ](#mandatory-mappings) hierboven.
...
* **Primaire aanpassing**: De vraag gebruikt `externalPersonId` als primaire herkenningsteken voor persoon die aanpassing.
...
* **Geadviseerde gebieden**: Terwijl slechts `email` en `externalPersonId` worden vereist, adviseert Adobe het in kaart brengen van alle beschikbare gebieden van de geadviseerde in kaart brengen lijst hierboven, om passende nauwkeurigheid en campagneprestaties te verbeteren.
