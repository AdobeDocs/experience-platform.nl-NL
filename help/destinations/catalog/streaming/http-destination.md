...
* Als u mTLS wilt gebruiken: voor uw gegevens die eindpunt ontvangen, moet TLS zijn uitgeschakeld en alleen mTLS zijn ingeschakeld. Als u OAuth 2 authentificatie ook gebruikt, moet u een afzonderlijk standaardHTTPS eindpunt voor symbolische herwinning handhaven. Zie de [ mTLS overwegingen ](#mtls-considerations) sectie voor details.
...
[!DNL mTLS] is een wederzijds authentificatieprotocol dat ervoor zorgt dat beide partijen die informatie delen zijn wie zij beweren te zijn alvorens de gegevens worden gedeeld. [!DNL mTLS] bevat een extra stap in vergelijking met standaard [!DNL TLS] , waarin de server ook om het certificaat van de client vraagt en dit verifieert, terwijl de client het certificaat van de server verifieert.
...
**mTLS wordt niet gesteund voor OAuth 2 authentificatieeindpunten:**

* De **[!UICONTROL Access Token URL]** die wordt gebruikt in OAuth 2 Client Credentials of OAuth 2 Password-verificatie ondersteunt mTLS niet
* Verzoeken voor ophalen en vernieuwen van token worden verzonden via standaard-HTTPS zonder verificatie van clientcertificaten

**Vereiste Architectuur:** als u mTLS voor uw gegevens nodig hebt die eindpunt ontvangen en OAuth 2 authentificatie gebruiken, moet u twee afzonderlijke eindpunten handhaven:

* **eindpunt van de Authentificatie:** StandaardHTTPS (zonder mTLS) voor symbolisch beheer
* **Gegevens die eindpunt ontvangen:** HTTPS met mTLS-slechts toegelaten voor profieluitvoer

Deze architectuur is een huidige platformbeperking. Ondersteuning voor mTLS op eindpunten van verificatie wordt geëvalueerd voor toekomstige releases.

### mTLS configureren voor gegevensexport {#configuring-mtls}

Om [!DNL mTLS] met [!DNL HTTP API] bestemmingen te gebruiken, **[!UICONTROL HTTP Endpoint]** (gegevens die eindpunt ontvangen) u in de [ bestemmingsdetails ](#destination-details) pagina vormt moet [!DNL TLS] gehandicapte protocollen hebben en slechts [!DNL mTLS] toegelaten. Als het [!DNL TLS] 1.2 protocol nog op het eindpunt wordt toegelaten, wordt geen certificaat verzonden voor de cliëntauthentificatie. Dit betekent dat als u [!DNL mTLS] wilt gebruiken met uw [!DNL HTTP API] -doel, uw gegevens die servereindpunt ontvangen, een [!DNL mTLS] -alleen ingeschakeld verbindingspunt moeten zijn.
...
>[!NOTE]
>
>**mTLS beperking:** [!UICONTROL Access Token URL] steunt mTLS niet. Als u mTLS voor uw gegevens wilt gebruiken die eindpunt ontvangen, moet uw authentificatieeindpunt standaardHTTPS gebruiken. Zie de [ mTLS overwegingen ](#mtls-considerations) sectie voor meer details over de vereiste architectuur.

* **[!UICONTROL Access Token URL]**: De URL aan uw zijde die toegangstokens uitgeeft en, naar keuze, tokens vernieuwt. Dit eindpunt moet standaard HTTPS gebruiken en ondersteunt geen mTLS.
...
>title="HTTP-eindpunt"
>abstract="De URL van het HTTP-eindpunt waarnaar u de profielgegevens wilt verzenden. Dit zijn uw gegevens die eindpunt ontvangen en steunt mTLS indien gevormd. Dit staat los van de OAuth 2 Token URL van de Toegang, die geen mTLS steunt."
...
* **[!UICONTROL HTTP Endpoint]**: De URL van het HTTP-eindpunt waarnaar u de profielgegevens wilt verzenden. Dit zijn uw gegevens die eindpunt ontvangen. Als u mTLS gebruikt, moet voor dit eindpunt TLS zijn uitgeschakeld en moet alleen mTLS zijn ingeschakeld. Merk op dat dit van OAuth 2 Token URL verschilt die tijdens authentificatie wordt gevormd.
