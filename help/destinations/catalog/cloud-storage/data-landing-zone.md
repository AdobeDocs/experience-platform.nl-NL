...
[!DNL Data Landing Zone] is een interface voor cloudopslag die wordt geleverd door [!DNL Adobe Experience Platform] en waarmee u toegang hebt tot een veilige, op de cloud gebaseerde opslagfaciliteit voor het opslaan van bestanden om bestanden uit Experience Platform te exporteren. U hebt toegang tot één [!DNL Data Landing Zone] container per sandbox en het totale gegevensvolume voor alle containers is beperkt tot de totale gegevens die worden geleverd bij uw Experience Platform-licentie voor producten en services. Alle klanten van Experience Platform en de bijbehorende toepassingen, zoals [!DNL Customer Journey Analytics] , [!DNL Journey Orchestration] , [!DNL Intelligent Services] en [!DNL Real-Time Customer Data Platform] , beschikken over één [!DNL Data Landing Zone] container per sandbox.
...
## Verifiëren voor de gegevenslandingszone die is ingericht in Azure Blob {#authenticate-dlz-azure}

>[!AVAILABILITY]
>
>Deze sectie is van toepassing op implementaties van Experience Platform die op Microsoft Azure worden uitgevoerd. Meer over de gesteunde infrastructuur van Experience Platform leren, zie het [&#x200B; multi-wolkenoverzicht van Experience Platform &#x200B;](https://experienceleague.adobe.com/nl/docs/experience-platform/landing/multi-cloud).
...
Als u uw gegevens via een openbare internetverbinding wilt beveiligen, gebruikt u SAS-verificatie voor een veilige toegang tot uw [!DNL Data Landing Zone] -container. Er zijn geen netwerkveranderingen vereist voor u om tot uw [!DNL Data Landing Zone] container toegang te hebben, wat betekent u geen lijsten van gewenste personen of dwars-regio montages voor uw netwerk hoeft te vormen.

### Sluit de [!DNL Data Landing Zone] -container aan op [!DNL Azure Storage Explorer] {#connect-container-to-storage-explorer}
...
| --- | --- |
| `dlz_destination` | Met het type `dlz_destination` kan de API een doelcontainer van de landingszone onderscheiden van de andere typen containers die voor u beschikbaar zijn. |
| `refresh` | Met de handeling `refresh` worden de gegevens van de landingszone opnieuw ingesteld en wordt automatisch een nieuwe `SASToken` gegenereerd. |
