...
description: Maak een live uitgaande verbinding met uw Azure Blob-opslag om CSV-gegevensbestanden periodiek vanuit Adobe Experience Platform te exporteren.
...
* **[!UICONTROL Connection string]**: de verbindingstekenreeks is vereist voor toegang tot gegevens in de blob-opslag. Het patroon van de [!DNL Blob] verbindingstekenreeks begint met: `DefaultEndpointsProtocol=https;AccountName={ACCOUNT_NAME};AccountKey={ACCOUNT_KEY}` .
   * Voor meer informatie over het vormen van uw [!DNL Blob] verbindingskoord, zie [ een verbindingskoord voor een de opslagrekening van Azure ](https://learn.microsoft.com/en-us/azure/storage/common/storage-configure-connection-string#configure-a-connection-string-for-an-azure-storage-account) in de documentatie van Microsoft vormen.
