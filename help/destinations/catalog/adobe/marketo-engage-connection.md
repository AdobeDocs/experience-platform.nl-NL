...
Gebruik deze bestemming voor synchronisatie in real time van publieksgegevens en profielattributen tussen [!DNL Adobe Experience Platform] en Marketo Engage.

## Gebruiksscenario&#39;s {#use-cases}

Om u beter te helpen begrijpen hoe en wanneer u de [!DNL Marketo Engage] bestemming zou moeten gebruiken, zijn hier voorbeelden van gebruiksgevallen die [!DNL Adobe Experience Platform] klanten kunnen oplossen door deze bestemming te gebruiken.
...
Het marketingteam wil een publiek met belangstelling voor een product activeren voor een nieuwe campagne. Veel van de profielen bestaan al in Marketo, maar sommige zijn nieuw en komen alleen voor in [!DNL Real-Time CDP] . Voor de bestaande mensen willen ze ervoor zorgen dat ze die mensen in Marketo bijwerken, maar ze willen ook nieuwe profielen maken.

Ze kunnen hun publiek in Marketo Engage activeren en het synchronisatietype **[!UICONTROL Audience and Profile]** gebruiken in combinatie met de handeling **[!UICONTROL Update existing and create new persons]** om ervoor te zorgen dat ze zich richten op bestaande leads uit Marketo en nieuwe maken voor het nieuwe publiek dat wordt geëxporteerd uit [!DNL Real-Time CDP] .
...
* Alleen Marketo Engage-instanties in dezelfde Adobe [!DNL Real-Time CDP] -organisatie zijn beschikbaar bij het instellen van deze bestemming.
...
* **[!UICONTROL Partition]**: *de selectie van de Verdeling is beschikbaar slechts wanneer het kiezen **[!UICONTROL Profile only]**of **[!UICONTROL Audience and profile]**synchronisatietypen*. Selecteer een Marketo-partitie-id die is gekoppeld aan uw gekozen werkruimte. Hiermee geeft u op welke partitie met leads in Marketo de geëxporteerde gegevens ontvangt. Als u geen specifieke partitie kiest, worden uw gegevens naar de **[!UICONTROL Default]** -partitie in Marketo verzonden.
...
Als u de verwachte gegevens niet ziet, controleert u de toewijzing- en exportinstellingen in [!DNL Adobe Experience Platform] en probeert u het exporteren opnieuw.
